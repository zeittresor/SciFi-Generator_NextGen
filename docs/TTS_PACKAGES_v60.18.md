# SciFi-Generator v60.18 — TTS complete packages
...
v60.18 contains package definitions for:
...
For small multi-speaker/style models the individual speaker IDs are shown as separate voice entries after installation. Very large speaker maps such as MLS are deliberately represented by one normal selector entry using the model's default speaker so the combo box does not gain hundreds of items.
...
The same installed package can be used by **Story als Audiodatei speichern …**. Piper synthesis is available on supported Windows and Linux hosts; narration volume and the bridge background are mixed by the application's audio mixer before WAV/MP3 output.
